...
serve(async () => {
  ...
  for (const row of data || []) {
    ...
    const message = `MACOKASA reminder: ${operator.fullName}, your membership ${operator.membershipNumber} expires in ${daysLeft} day(s). Renew by AirtelMoney, Mpamba, bank card, EFT, cash office receipt, or visit a MACOKASA office.`;
    queued.push({
